...
class UncompressedSmallFile extends AbstractVirtualComponent implements IVirtualComponent {
    ...
    public function __construct(string $fileName, IFile $file) {
        ...
        // add new content
        $this->_file->write(ZipConstants::LOCAL_FILE_SIGNATURE);
        $this->_file->write(ZipConstants::VERSION_REQUIRED);
        $this->_file->write(ZipConstants::GENERAL_FLAGS);
        $this->_file->write(ZipConstants::COMPRESSION_METHOD);
        $this->_file->write(ZipConstants::LAST_MODIFIED_TIME);
        $this->_file->write(ZipConstants::LAST_MODIFIED_DATE);
        $this->_file->write(pack("V", $this->_metadata['crc']));
        $this->_file->write(pack("V", $this->_metadata['compressedSize']));
        $this->_file->write(pack("V", $this->_metadata['uncompressedSize']));
        $this->_file->write(pack("v", strlen($this->_metadata['fileName'])));
        $this->_file->write(pack("v", 0));
        $this->_file->write($this->_metadata['fileName']);
